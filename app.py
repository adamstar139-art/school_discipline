...
# Custom Responsive Mobile & Desktop Styling
st.markdown("""
<style>
...
DEFAULT_STUDENTS = [
    ...
    ('1170582166', 'يوسف محمد عبدالله الدوسري', 'الصف الأول المتوسط', 'فصل 2'),
...
def init_db():
    ...
    # UNCONDITIONAL INSERT OR IGNORE TO GUARANTEE ALL 167 STUDENTS ALWAYS EXIST
...
# Always initialize on script import
init_db()
...
def fetch_students(grade=None, section=None):
    ...
    # Auto-healing: if empty for requested grade/section, re-seed unconditionally and fetch again
    if df.empty and grade and section:
        c = conn.cursor()
        c.executemany("INSERT OR IGNORE INTO students (id, name, grade, section) VALUES (?, ?, ?, ?)", DEFAULT_STUDENTS)
        conn.commit()
        df = pd.read_sql_query(query, conn, params=params)
...
# PAGE 1: Teacher Screen (Dynamic interactive UI)
if page == "👨‍🏫 شاشة المعلم (رصد مخالفة)":
    st.subheader("📋 شاشة المعلم - رصد المخالفة السلوكية")
    st.info("💡 اختر الصف والفصل أولاً لتحديث قائمة الطلاب المنسدلة تلقائياً.")
    ...
    with col2:
        ...
        if student_options:
            ...
            st.warning(f"⚠️ لا يوجد طلاب مسجلون حالياً في ({selected_grade} - {selected_section}).")
            if st.button("🔄 إدراج واستعادة جميع الطلاب الافتراضيين (167 طالب)", key="restore_st_btn"):
                init_db()
                st.rerun()
...
# PAGE 4: Student Management (Open Access)
    ...
    with m_tab0:
        ...
        if st.button("🔄 إعادة تحميل وتأكيد الطلاب الافتراضيين (167 طالب)", key="reseed_btn"):
            init_db()
...
# PAGE 5: Printing & Exporting Reports
    ...
    if inc_df.empty:
        ...
        # Display Official Responsive Report Layout
        st.markdown(f"""
